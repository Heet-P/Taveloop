...
import { getTrips, searchCities } from "@/lib/api";
...
function CityPin({ city }: { city: City }) {
  return (
    <motion.div
      ...
      className="bg-[var(--bg-card)] border border-[var(--border)] rounded-[var(--radius-lg)] p-4 shadow-[var(--shadow-card)] cursor-pointer"
    >
      ...
      <div className="flex items-center gap-1 mt-2">
        {"★★★★★".split("").map((_, i) => (
          <span key={i} className={`text-xs ${i < Math.round(city.popularityScore) ? "text-[var(--accent-yellow)]" : "text-[var(--border)]"}`}>★</span>
        ))}
        <span className="text-xs text-[var(--text-muted)] ml-1">{city.popularityScore.toFixed(1)}</span>
      </div>
    </motion.div>
  );
}
  ...
  return (
    <PageWrapper>
      <div className="space-y-10">
        ...
        {/* Top Destinations */}
        <section>
          <h2 className="text-xl font-semibold text-[var(--text-primary)] mb-4" style={{ fontFamily: "var(--font-fraunces, Georgia, serif)" }}>
            Top Destinations
          </h2>
